# -*- coding: utf-8 -*-
"""
extract_fat.py -- FAT 1급 기출문제(comcbt.com, 한국공인회계사회 원본)를
questions.json에 합칠 항목으로 변환.

comcbt.com 게시물 하나에 PDF 2개가 첨부돼 있는데(문제만/해설포함), 더 큰
쪽(해설포함)에 문제+보기+정답+해설이 전부 들어있어서 그것 하나만 쓴다.
"실무수행평가"(회계프로그램 데이터입력 시뮬레이션)는 실제 프로그램 없이는
학습 의미가 없어서 "실무이론평가"(객관식 15문항)만 추출한다.

[ 사용법 ]
    python extract_fat.py --source "../FAT1급_기출문제" --merge-into questions.json
"""
import argparse
import glob
import json
...
def process_round(pdf_path):
    ...
    for num, q in questions.items():
        results.append({
            "id": f"FAT1급|{round_label}|theory|{num}",
            "exam": "FAT1급",
            "subject": "FAT",
            "level": "1급",
            "type": "theory",
...
def main():
    ...
    parser.add_argument("--source", required=True, help="FAT1급_기출문제 PDF들이 있는 폴더")
    ...
    existing = [q for q in existing if q.get("exam") != "FAT1급"]  # 재실행 시 중복 방지

    merged = existing + all_new
    with open(args.merge_into, "w", encoding="utf-8") as f:
        json.dump(merged, f, ensure_ascii=False, indent=2)

    print(f"\nFAT1급 {len(all_new)}문제 추가 -> 총 {len(merged)}문제")
